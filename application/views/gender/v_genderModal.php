...
<div id="genderModal" class="modal fade">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">×</button>
				<h5 class="modal-title"><?= lang('gender_form') ?></h5>
			</div>
			...
			<div class="modal-body">
				...
				<div class="form-group">
					<label class="col-lg-3 control-label"><?= lang('gender_name') ?></label>
					<div class="col-lg-9">
						<input type="text" name="gender_name" id="gender_name" class="form-control"
							   placeholder="Enter <?= lang('gender_name') ?>">
...
<script>
	...
	$(document).on('hide.bs.modal', '#genderModal', function () {
		$('#genderTable input').prop('checked', false);
		CheckboxKeyGen();
	});
	...
	$(document).ready(function () {
		...
		$('.addGender').click(function () {
			DtFormClear('genderDetails');
			$("form#genderDetails input[name=gender_id]").val('');
		...
		var validator = $("#genderDetails").validate({
			...
			rules: {
				gender_name: {
					required: true,
					remote: {
						url: "<?php echo site_url( "Gender/NameExist");?>",
						type: "post",
						data: {
			...
			messages: {
				gender_name: {
					required: "Please Enter <?= lang('gender_name') ?>",
					remote  : "<?= lang('gender_name') ?> Already Exist",
							...
							swal({
								...
								function () {
									if(typeof dt_DataTable !== 'undefined' ) {
										...
										var option = new Option(resObj.gender_name, resObj.gender_id, true, true);
										$('#GenderList').append(option).trigger('change');
									}
								});
						}else{
							swal({
	...
	$(document).on('click', '.editRecord', function () {
		$('#genderTable input[class="dt-checkbox styled"]').prop('checked', false);
		$("form#genderDetails .validation-error-label").html('');
		...
		DtFormFill('genderDetails', aData);
