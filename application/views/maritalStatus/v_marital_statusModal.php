...
<div id="maritalStatusModal" class="modal fade">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">×</button>
				<h5 class="modal-title"><?= lang('marital_status_form') ?></h5>
			</div>
			...
			<div class="modal-body">
				...
				<div class="form-group">
					<label class="col-lg-3 control-label"><?= lang('marital_status_name') ?></label>
					<div class="col-lg-9">
						<input type="text" name="marital_status_name" id="marital_status_name" class="form-control"
							   placeholder="Enter <?= lang('marital_status_name') ?>">
...
<script>
	...
	$(document).on('hide.bs.modal', '#maritalStatusModal', function () {
		$('#maritalStatusTable input').prop('checked', false);
		CheckboxKeyGen();
	});
	...
	$(document).ready(function () {
		...
		$('#maritalStatusModal').on('shown.bs.modal', function () {
			$('#marital_status_name').focus();
		});

		//country model open
		$('.addMaritalStatus').click(function () {
			DtFormClear('maritalStatusDetails');
			$("form#maritalStatusDetails input[name=marital_status_id]").val('');
			$('#maritalStatusModal').modal('show');
		});
		...
		var validator = $("#maritalStatusDetails").validate({
			...
			rules: {
				marital_status_name: {
					required: true,
					remote: {
						url: "<?php echo site_url( "MaritalStatus/NameExist");?>",
						type: "post",
						data: {
			...
			messages: {
				marital_status_name: {
					required: "Please Enter <?= lang('marital_status_name') ?>",
					remote  : "<?= lang('marital_status_name') ?> Already Exist",
							...
							swal({
								...
								function () {
									if(typeof dt_DataTable !== 'undefined' ) {
										...
										var option = new Option(resObj.marital_status_name, resObj.marital_status_id, true, true);
										$('#MaritalStatusList').append(option).trigger('change');
									}
								});
						}else{
							swal({
		...
		if (selectedLength == 0) {
			...
			swal({
				...
				function () {
					$.ajax({
						type: "post",
						url: "<?= base_url("MaritalStatus/delete")?>",
						dataType: "json",
	...
	$(document).on('click', '.editRecord', function () {
		$('#maritalStatusTable input[class="dt-checkbox styled"]').prop('checked', false);
		$("form#maritalStatusDetails .validation-error-label").html('');
		...
		DtFormFill('maritalStatusDetails', aData);
