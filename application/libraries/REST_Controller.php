...
abstract class REST_Controller extends CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
       ...
       // $this->load->model(array('WebServiceModel'));
        ...
        $ignoreMethods = array('login','checkOtp','getLanguageList');
        $functionMethod = $this->uri->segment(3);

        if(isset($functionMethod) && !in_array($functionMethod,$ignoreMethods)) {

            $this->load->model(array('Mdl_member'));
            $memberId = (isset($_POST['member_id']) && $_POST['member_id'] != '') ? $_POST['member_id'] : '';
            $authToken = (isset($_POST['auth_token']) && $_POST['auth_token'] != '') ? $_POST['auth_token'] : '';

            $tokenData = $this->Mdl_member->getMemberTokenData($memberId, $authToken);
